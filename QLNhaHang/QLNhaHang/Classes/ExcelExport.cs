...
namespace QLNhaHang.Classes
{
    public class ExcelExport
    {
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer, string tongCong)
        {
            if (pReplacer != null)
            {
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
            ...
            switch (viewName)
            {
                ...
                case "ThongKe":
                    ...
                    arrByte = File.ReadAllBytes("ThongKe.xlsx").ToArray();
                    break;
                case "ThongKeMon":

                    arrByte = File.ReadAllBytes("ThongKeMon.xlsx").ToArray();
                    break;
                case "PhieuNhap":
        ...
        //public bool ExportKhoa(List<Khoa> dataSource, ref string fileName, bool isPrintPreview, string tongCong)
        //{
        //    // Check if data is null
        ...
        //    BuildReplacerCurrentDate(ref replacer, tongCong);

        //    return OutSimpleReport(dataSource, replacer, "DanhMucKhoa", isPrintPreview, ref fileName);
        //}

        public bool ExportThongKeThang(List<Chart> dataSource, ref string fileName, bool isPrintPreview, string tongCong, string ngayBD, string ngayKT, string tieuDe,string column1, string column2, string column3)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            // Create replacer
            Dictionary<string, string> replacer = new Dictionary<string, string>();
            replacer.Add("%NgayBD", ngayBD);
            replacer.Add("%NgayKT", ngayKT);
            replacer.Add("%TieuDe", tieuDe);
            replacer.Add("%TongCong", tongCong);
            replacer.Add("%Ngay", DateTime.Now.Day.ToString());
            replacer.Add("%Thang", DateTime.Now.Month.ToString());
            replacer.Add("%Nam", DateTime.Now.Year.ToString());
            replacer.Add("%Column1", column1);
            replacer.Add("%Column2", column2);
            replacer.Add("%Column3", column3);

            //    replacer.Add(V_TONGSO, "100");

            //BuildReplacerCurrentDate(ref replacer, tongCong);

            return OutSimpleReport(dataSource, replacer, "ThongKe", isPrintPreview, ref fileName);
        }
        public bool ExportThongKeMon(List<Chart> dataSource, ref string fileName, bool isPrintPreview, string tong, string giam, string tongCong, string ngayBD, string ngayKT)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            replacer.Add("%Thang", DateTime.Now.Month.ToString());
            replacer.Add("%Nam", DateTime.Now.Year.ToString());
            replacer.Add("%Tong", tong);
            replacer.Add("%TongGiaGiam", giam);

            //    replacer.Add(V_TONGSO, "100");

            //BuildReplacerCurrentDate(ref replacer, tongCong);

            return OutSimpleReport(dataSource, replacer, "ThongKeMon", isPrintPreview, ref fileName);
        }
        public bool ExportPhieuNhap(List<CTPNExport> dataSource,PhieuNhap phieuNhap,  ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            // Create replacer
            Dictionary<string, string> replacer = new Dictionary<string, string>();
            replacer.Add("%TongTien", dataSource.Sum(t => t.ThanhTien).ToString());
            replacer.Add("%PhieuNhap.NhaCungCap.TenNCC", phieuNhap.NhaCungCap.TenNCC);
            replacer.Add("%PhieuNhap.NhaCungCap.DiaChi", phieuNhap.NhaCungCap.DiaChi);
            replacer.Add("%PhieuNhap.NhaCungCap.SDT", phieuNhap.NhaCungCap.SDT);
            replacer.Add("%PhieuNhap.MaNV", phieuNhap.NguoiDung.HoTen);
            replacer.Add("%PhieuNhap.MaPN", phieuNhap.MaPN.ToString());
            replacer.Add("%NgayThangNamIn", DateTime.Now.ToString());
